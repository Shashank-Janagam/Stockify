...
const AIStockReport: React.FC<AIStockReportProps> = ({ symbol }) => {
  ...
  const isIndex = symbol.startsWith('^') || ["NIFTY_MIDCAP_100.NS", "NIFTY_FIN_SERVICE.NS"].includes(symbol.toUpperCase());
  const showPanel = isIndex || isHovered;
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      try {
        ...
        const response = await fetch(`${HOST}/api/stocks/${symbol}/ai-report`);
  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: loading && !showPanel ? 0.3 : 1
      }}
    >
      ...
      <div style={{
        width: showPanel && report ? '480px' : '0px',
        height: '780px',
        backgroundColor: 'rgba(255, 255, 255, 0.99)',
        borderLeft: showPanel && report ? '1px solid rgba(226, 232, 240, 1)' : 'none',
        overflow: 'hidden',
        transition: 'all 0.6s cubic-bezier(0.16, 1, 0.3, 1)',
        display: 'flex',
        flexDirection: 'column',
