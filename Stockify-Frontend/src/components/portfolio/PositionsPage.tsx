...
function getRoute(symbol: string, name: string) {
    ...
    return (s.endsWith(".NS") || s.endsWith(".BO")) ? `/stocks/${s}/${slugify(name)}` : `/us/${s}/${slugify(name)}`;
}
...
const PositionsPage: React.FC = () => {
        ...
        return (
            <React.Fragment key={p.key}>
                ...
                >
                    ...
                    <td>
                        <div className="hp-2line">
                            <span className="hp-primary">
                                {p.totalQty} shares
                                {p.lotCount > 1 && (
                                    <span style={{ marginLeft: '6px', fontSize: '10px', backgroundColor: '#f3f4f6', padding: '2px 4px', borderRadius: '4px', color: '#4b5563', fontWeight: '600' }}>
                                        {p.lotCount} lots
                                    </span>
                                )}
                            </span>
                    ...
                    <td>
                        <div className={`hp-pnl ${rowProfit ? "hp-profit" : "hp-loss"}`}>
                            ...
                            <span className="hp-sm">{rowProfit ? "+" : ""}{p.totalPnlPct}%</span>
                        </div>
                    </td>
                ...
                {drawer?.symbol === p.symbol && drawer?.productType === p.productType && (
                    <tr className="h-drawer-row" onClick={e => e.stopPropagation()}>
    ...
    const sectionHeader = (label: string, rows: MergedPosition[]) => {
        ...
        return (
            <tr key={`sh-${label}`} className="hp-section-header-row">
                <td colSpan={7}>
