...
class BSEQuantEngine:
    ...
    def get_delivery_percentage(self, ticker: str):
        """
        Fetches the latest available Delivery to Traded Quantity percentage from BSE.
        """
        ...
        # Search backwards to find the latest available daily delivery report
    ...
    def get_put_call_ratio(self, ticker: str):
        """
        Fetches option chain data. Since BSE has low option liquidity for individual stocks,
        this falls back to a standard derivative PCR.
        """
    ...
    def get_relative_strength_vs_nifty(self, ticker: str, periods=20):
        """
        Calculates how much the stock is outperforming the Nifty 50 benchmark over the last N days.
        """
        try:
            # We use NSE historical data (.NS) here because yfinance coverage
            # for BSE (.BO) historical prices is often incomplete or empty.
            ...
            # Align MultiIndex columns if present by extracting raw float values
    ...
    def generate_agent_payload(self, ticker: str):
        """
        Compiles the advanced metrics into a JSON payload for the Groq LLM Agent.
        """
...
if __name__ == "__main__":
    ...
    quant_state = bse_engine.generate_agent_payload(target_stock)
    
    import json
    print("\n--- Final Payload for Technical Agent ---")
    print(json.dumps(quant_state, indent=2))